...
<template>
  <aside
    v-if="isLeftSidebarMenuOpen"
    v-motion
    :initial="{ opacity: 0, x: -100 }"
    :enter="{ opacity: 1, x: 0 }"
    :delay="200"
    class="fixed shrink-0 hidden lg:block top-0 left-0 w-[20rem] bg-[#161b21] h-screen text-gray-100 text-sm px-4 py-3 rounded-r-2xl shadow-xl z-40"
  >
    ...
    <ul
      class="absolute bg-[#161b21] z-20 bottom-0 right-0 left-0 p-2 space-y-1"
    >
      <hr class="border-gray-800" />
      <li class="hover:bg-gray-800 p-2 hover:rounded-md">
        <NuxtLink href="#" class="flex items-center gap-2">
