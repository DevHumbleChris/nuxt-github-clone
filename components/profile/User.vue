  ...
  <section class="bg-light-dark">
    <div class="grid grid-cols-8 text-sm">
        <div class="col-span-2 p-5 space-y-3 text-gray-200">
            ...
            <div class="space-y-4">
                <div>
                    <h2 class="text-xl text-white font-semibold">{{ userInfo?.name }}</h2>
                    <h1 class="text-gray-500">{{ userInfo?.login }}</h1>
                </div>
                <button class="block bg-[#21262c] p-2 w-full rounded-md">follow</button>
            </div>
            <p>{{ userInfo?.bio }}</p>
            <div class="flex items-center gap-2">
              <NuxtLink to="#" class="block flex items-center gap-1 group">
                <Icon name="octicon:people-24" class="w-4 h-auto group-hover:text-blue-500" />
                <p class="text-gray-500 group-hover:text-blue-500"><span class="text-gray-200 group-hover:text-blue-500">{{ userInfo?.followers }}</span> followers</p>
              </NuxtLink>
              <div class="w-1 h-1 bg-white rounded-full"></div>
              <NuxtLink to="#" class="block group">
                <p class="text-gray-500 group-hover:text-blue-500"><span class="text-gray-200 group-hover:text-blue-500">{{ userInfo?.following }}</span> following</p>
              </NuxtLink>
            </div>
            <div class="space-y-2">
              <div class="flex items-center gap-2">
